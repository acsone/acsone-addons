...
class TestCagnotteSale(WalletCommon):
    ...
    def _get_line_values(self, discount=False):
        ...
        if discount:
            vals.update(
                {
                    "price_unit": self.sale.amount_total + self.wallet.balance + 100.0,
                    "product_uom_qty": -1.0,
                }
            )
        return vals
    ...
    def _create_order_line_from_sale(self, discount=False):
        vals = self._get_line_values(discount)
        self.sale.write({"order_line": [(0, 0, vals)]})
    ...
    def test_wallet_sale_reapply_on_line_add(self):
        ...
        self.assertEqual(len(lines_before) + 2, len(self.sale.order_line))
        line.write(
            {
                "discount": 50,
            }
        ...
        self.sale.unlink()
        self.assertEqual(15.0, self.wallet.balance)
    ...
    def test_wallet_sale_multiple_reapply_on_line_add(self):
        ...
        self.assertEqual(len(lines_before) + 3, len(self.sale.order_line))
        self.assertEqual(2, len(self.sale.order_line.filtered("account_wallet_id")))
    ...
    def test_wallet_sale_discount_line(self):
        ...
        self.assertEqual(len(lines_before) + 1, len(self.sale.order_line))
        self.assertEqual(-15.0, line_wallet.price_total)
        self.sale.write({"order_line": [(1, line.id, {"discount": 50})]})
        line_wallet = self.sale.order_line.filtered("account_wallet_id")
        self.assertEqual(-7.5, line_wallet.price_total)
    ...
    def test_wallet_sale_invoiced(self):
        ...
        invoice.action_post()
        wallet_line = invoice.invoice_line_ids.filtered("account_wallet_id")
        self.assertEqual(1, len(wallet_line))
        wallet_move_line = invoice.invoice_line_ids.filtered("account_wallet_id")
        self.assertEqual(1, len(wallet_move_line))
        self.wallet.invalidate_cache()
        self.assertEqual(0.0, self.wallet.sale_order_balance)
        self.assertEqual(0.0, self.wallet.balance)

    def test_wallet_sale_cancelled(self):
        self._provision_wallet(15.00)
        lines_before = self.sale.order_line
        self.sale.apply_wallet(self.wallet)
        self.assertEqual(1, len(self.wallet.sale_order_line_not_invoiced_ids))
        self.assertEqual(1, len(self.wallet.sale_order_line_ids))
        self.assertEqual(len(lines_before) + 1, len(self.sale.order_line))
        self.assertEqual(-15.00, self.wallet.sale_order_balance)
        self.assertEqual(
            0.00,
        ...
        self.assertEqual(1, len(self.wallet.sale_order_line_not_invoiced_ids))
        self.assertEqual(1, len(self.wallet.sale_order_line_ids))
        self.assertEqual(0.00, self.wallet.sale_order_balance)
