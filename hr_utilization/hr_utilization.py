...
class res_company(osv.Model):
    ...
    _columns = {
        'fulltime_calendar_id' : fields.many2one("resource.calendar", "Full-time Calendar", required=False,
            help="Calendar used as the full working time reference of the company."),
    }
...
class resource_calendar_leaves(osv.Model):
    ...
    def _determin_year_from_date_from(self, cr, uid, ids, field, arg, context=None):
        result = {}
...
class hr_utilization_configuration(osv.Model):
    ...
    def _column_list(self, cr, uid, ids, field, arg, context=None):
        ...
        for configuration in configurations :
            result[configuration.id] = self._get_column_list(configuration.configuration_column_ids)
        return result
    ...
    def copy(self, cr, uid, id, defaults, context=None):
        ...
        n = 0
        while True:
            existing_ids = self.search(cr, uid, [('name', '=', new_name)], context=context)
            if not existing_ids:
                break
            n += 1
            new_name = '%s (%s)' % (copy_name, n)
        defaults['name'] = new_name
        return super(hr_utilization_configuration,self).copy(cr, uid, id, defaults, context=context)

    _columns = {
        'name': fields.char('Utilization Configuration Name', size=128, required=True),
        'with_fte': fields.boolean('With Full-time Equivalent Column', required=True),
        'configuration_column_ids': fields.one2many('hr.utilization.configuration.column', 'configuration_id', 'List of Columns to Print'),
        'column_list': fields.function(_column_list, type='string', string='Columns'), 
    }
    _defaults = {
        'with_fte': True,
    }  
...
class hr_utilization_configuration_column(osv.Model):
    ...
    _rec_name = 'column_id'
    _columns = {
        'column_id': fields.many2one('hr.utilization.column','Column', required=True, ondelete='cascade'),
        'configuration_id': fields.many2one('hr.utilization.configuration', 'Configuration', required=True, ondelete='cascade'),
...
class hr_utilization_column(osv.Model):
    ...
    def _get_analytic_account_list(self, cr, uid, analytic_account_ids, context):
        ''' Build a list of analytic accounts codes '''
        analytic_account_obj = self.pool.get('account.analytic.account')
        analytic_account_names = analytic_account_obj.name_get(cr, uid, analytic_account_ids, context)
    ...
    def _analytic_account_list(self, cr, uid, ids, field, arg, context=None):
        ...
        for column in columns :
            result[column.id] = self._get_analytic_account_list(cr, uid, [a.id for a in column.analytic_account_ids], context)
        return result
    ...
    def copy(self, cr, uid, id, defaults, context=None):
        ...
        while True:
            ...
            existing_ids = self.search(cr, uid, ['|', ('name', '=', new_name), ('short_name', '=', new_short_name)], context=context)
            if not existing_ids:
                break
            n += 1
            new_name = '%s (%s)' % (copy_name, n)
        ...
        return super(hr_utilization_column,self).copy(cr, uid, id, defaults, context=context)
    ...
    _columns = {
        ...
        'analytic_account_list': fields.function(_analytic_account_list, type='string', string='Analytic accounts'), 
    }
    _sql_constraints = [
        ('unique_name', 'unique(name)', 'Column name must be unique'),
        ('unique_name', 'unique(short_name)', 'Column short name must be unique'),
    ]
    _order = "name"
