...
class PosWalletCommon(WalletCommon):
    ...
    def compute_tax(cls, product, price, qty=1, taxes=None):
        if not taxes:
            taxes = product.taxes_id.filtered(lambda t: t.company_id.id == cls.env.company.id)
    ...
    def setUpClass(cls):
        ...
        cls.pos_config.write({'payment_method_ids': [(4, cls.wallet_payment_method.id), (4, cls.bank_payment_method.id), (4, cls.cash_payment_method.id)]})
