    ...
    models.Paymentline = models.Paymentline.extend({
                ...
                if (pline.has_cagnotte()) {
                    if (pline.get_wallet_code() === code) {
                        Gui.showPopup('ErrorPopup',{
                            ...
                            'body': _t('The coupon code ' + code + ' is already use in this order.'),
                        });
                        return;
                    }
                }
        ...
        _get_coupon_values(code){
            ...
            return new Promise(function (resolve, reject) {
                self.order.pos.rpc({
                    model: 'account.wallet', method: 'search_read',
                    args: [domain, fields],
                        ...
                        if (wallet.length){
                            ...
                            Gui.showPopup('ErrorPopup',{
                                ...
                                'body': _t('The wallet code ' + code + ' is not usable' +
                                                '. Check payment method : ' + payment_method.name +
                                                '. Check customer'),
                            });
                            return;
                        };
        ...
        set_wallet: function(wallet){
            ...
            self.wallet_balance = round_di(parseFloat(wallet.balance) || 0, self.pos.currency.decimals);
            if (wallet.balance <= 0){
                wallet.balance = self.get_amount();
            }
            self.wallet_code = wallet.coupon_id[1];
            self.set_amount(Math.min(wallet.balance.toFixed(self.pos.currency.decimals), self.pos.get_order().get_due(self)));
        },
